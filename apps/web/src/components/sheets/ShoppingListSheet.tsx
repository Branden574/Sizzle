                ...
                <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16, overflow: 'hidden' }}>
                  {g.items.map((it) => (
                    <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderBottom: '1px solid var(--line)' }}>
                      ...
                      <span onClick={() => toggle(it.id)} style={{ flex: 1, fontSize: 15.5, color: it.checked ? 'var(--text-faint-2)' : 'var(--text-2)', textDecoration: it.checked ? 'line-through' : 'none', cursor: 'pointer' }}>{it.text}</span>

...
## Video-platform 100k-user audit + hardening — 2026-07-16

A 38-agent audit workflow (9 dimension audits → adversarial verification → synthesis) found **28 confirmed high/critical + 46 medium/low, 0 refuted**; rating "internal-testing ready". Full findings + the launch-readiness synthesis are the source of truth. **The #1 bottleneck was the video readiness path being synchronously coupled 1:1 to Cloudflare's account-wide API budget (1,200 req/5min), with four downstream amplifiers.**

**Automated App Store Connect uploads (NEW — mirrors StockPilot, adapted for Capacitor/xcodebuild):** `apps/web/scripts/` + npm scripts run from `apps/web`: `npm run release:ios` (build→archive→export→verify-distribution-signature→altool upload), `npm run asc:prepare` (poll processing→attach build→What's New→optional review notes via `--review-info`), `npm run asc:submit`, and `npm run release:ios:full` (all three). Config in `scripts/release.config.json` (no secrets; demo pw via `ASC_DEMO_PASSWORD` env). Runbook: `docs/app-store-auto-submit.md`. Auth (ASC API key) verified live. Note: v1.0/build 22 is currently WAITING_FOR_REVIEW (releaseType AFTER_APPROVAL).

**Fixed + verified this session (code compiles/builds; DB changes applied to hosted + live-verified):**
- **CRITICAL — RLS write lockdown** (`…001356`): `revoke insert,update,delete on recipes+video_assets from authenticated,anon`. Closed a moderation-bypass / URL-injection hole (any user could PATCH their own row's status/urls/counters via the public anon key). Live-verified: `set role authenticated` + owner JWT → denied. Web writes zero direct rows; all counter funcs are SECURITY DEFINER.
- **Video pipeline hardening** (`videoFinalize.ts`, `stream.ts`, `moderation.ts`, `uploads.ts`, `internal.ts` + cols `…001*` `source_url`/`last_polled_at`/`moderation_attempts`): honest `getAsset` (throws on 429/5xx, 404→error — no more masquerading as 'processing' and clobbering live posters); **fail-CLOSED** thumbnail moderation with a bounded retry (was fail-open in the CF-thumbnail-404 window); **poll-collapse** (8s freshness floor) decouples readiness from the CF API budget; **re-ingest** on `/copy` failure instead of publishing raw HEVC; abandon-sweep no longer bricks healthy long transcodes; CF fetch timeouts; swallowed cron/upload catches now log.
- **Scale fixes**: following-feed `following_feed_recipes` + `mutual_follow_ids` RPCs (killed the ~690-follow hard-500; DB-verified equal to the inline join); viewer-signal caps + chunked engaged `.in()` (heavy-user personalization no longer silently dies); indexes (`profiles(boost) partial`, `rate_limits(window_start)`, `reactions/saves(user_id,created_at)`, `recipes(cook_id,created_at)`); guest-feed edge cache (`s-maxage=30`).
- **Client feed/playback**: overlay playback suppression (feed video pauses under recipe/cook/upload/viewer sheets — kills double-audio + egress), `FeedCard` memoized, `VideoPlayer` src-detach on unmount, lazy far-card posters.
- **Reliability/cost/security**: engagement-events retention pg_cron (`sizzle-purge-engagement`, views>90d/impressions>30d); `/health` gains `stuckVideoBacklog`; webhook replay-freshness (300s); mock-provider prod boot-guard; per-user **daily** upload quota (50/day); **deletion lifecycle** — `deleteAsset()` + recipe-delete now tears down the CF Stream asset + Supabase Storage source/poster (App Store 5.1.1(v)/GDPR).

**DEFERRED — need iOS-simulator/browser verification (per rules 4/5) or Branden's accounts/config:**
1. **Branden's config (unblocks 2 criticals + the bottleneck fix):** set `SENTRY_DSN` on the `sizzle` (=API) project + `VITE_SENTRY_DSN` on `sizzle-api` (=web) — prod runs with ZERO telemetry while live Stripe money moves; register the Cloudflare Stream webhook + set `CLOUDFLARE_STREAM_WEBHOOK_SECRET` (makes the poll-collapse fix's webhook the primary signal); upgrade Supabase to **Pro** ($25 — free-tier 5GB egress dies in days from the /copy relay).
2. **Device/browser-verify before shipping:** feed virtualization (K, WKWebView jetsam ceiling — biggest remaining scale risk), the `['feed']` invalidation storm (L), tus resumable uploads (N — also raises the honest ~200MB web cap the UI overstates as 4K/30-min), view-log batching, Firebase lazy-import, cold-start persisted-feed cache, premium signed URLs (P — pricing is mutable post-upload, so sign-all or flip-on-price), account-purge media-teardown queue (recipe-delete teardown done; the auth-cascade-before-snapshot case needs a queue table), hot-row counter decomposition (R).
3. **Verification gate (rule 4/5):** publish ONE real Cloudflare video end-to-end in production (the public catalog is still 7 static seed MP4s — the pipeline being hardened has never carried a public prod asset).

Pre-existing (not from this session): the controls contract test flags `ImageCarousel.tsx` raw `<button>` arrows.
